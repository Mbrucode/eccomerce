 ...
 * Plugin URI: http://www.plugins.byconsole.com/product/byconsole-wooodt-extended
...
$order_id = version_compare( WC_VERSION, '3.0.0', '<' ) ? $order->id : $order->get_id();
	...
	if(get_post_meta( $order_id, 'byconsolewooodt_delivery_type', true )=='take_away'){
			...
			if(!empty(get_option('byconsolewooodt_takeaway_lable'))) 
			{ 
				$order_delivery_type =  get_option('byconsolewooodt_takeaway_lable'); 
			} 
			else 
			{ 
				$order_delivery_type = 'Take away';
			}
	...
	if(get_post_meta( $order_id, 'byconsolewooodt_delivery_type', true )=='levering'){
			...
			if(!empty(get_option('byconsolewooodt_delivery_lable'))) 
			{ 
				$order_delivery_type =  get_option('byconsolewooodt_delivery_lable'); 
			} 
			else 
			{ 
