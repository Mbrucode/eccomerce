    ...
    if ( empty($_POST['name']) || empty($_POST['email']) || empty($_POST['subject']) || empty($_POST['message']) ) {
        ...
        if (!wp_verify_nonce($_POST['_cf_nonce'], 'contact_form_html')) {
            ...
            if ($email_check == 1) {
                ...
                $subject_header = sprintf(esc_html__( 'New contact form message from : %1$s' , 'villenoir' ), $name);
                ...
                $message .= "<tr style='background: #eee;'><td><strong>".esc_html__( 'Message', 'villenoir')."</strong> </td><td>" . $messagecf . "</td></tr>";
                $message .= "</table>";
                $message .= "</body></html>";
                ...
                if ( wp_mail($to, $subject_header, $message, $header) ) {
                    $status = 'success';
                    $thankyou = '';
